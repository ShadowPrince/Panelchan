...
open class ILTranslucentView: UIView {
    ...
    fileprivate var _borders = UIEdgeInsetsMake(0, 0, 0, 0)
    @IBInspectable open var borderTop: Bool {
        set {
            if newValue {
                _borders.top = 1.0

                let top = CALayer()
                top.backgroundColor = self.borderColor.cgColor
        ...
        get {
            return _borders.top > 0.0
        }
    }

    @IBInspectable open var borderBottom: Bool {
        set {
            if newValue {
                _borders.bottom = 1.0
                ...
                bot.frame = CGRect(x: 0, y: self.frame.height - 1, width: self.frame.width, height: 1)
                self.layer.addSublayer(bot)
            }
        }

        get {
            return _borders.bottom > 0.0
        }
    }
